...
class SafetyADMC(VideoModel):
    """Abstract Discrete Markov Chain (ADMC) for safety properties ."""
    ...
    def __init__(
        ...
        """Discrete Real Time Markov Chain (DRTMC) Automaton for safety properties.
    ...
    def _update_probability_of_propositions(self, frame: Frame) -> None:
    ...
    def add_safety_states(self):
        ...
        safe_state_indices = []
        safe_state = VideoState(
            state_index=len(self.states),
            frame_index_in_automaton=self.frame_index_in_automaton,
            proposition_status_set="terminal",
            proposition_set=["safe"],
        )
        unsafe_state = VideoState(
        ...
        if self.safe_state:
            ...
            for i, transitions in enumerate(self.transitions):
                if transitions[0] == 0 or transitions[0] == 1:
